...
public class UserOperation {
	...
	public boolean userDataUpdate(UserData ud) throws SQLException {
		...
		try {
			...
			con.setAutoCommit(false);
			if (ud.getPassword() == null || ud.getNewPassword() == null) {
				...
				pass.next();
				if (BCrypt.checkpw(ud.getPassword(), pass.getString(1))) {

					String password = BCrypt.hashpw(ud.getNewPassword(), BCrypt.gensalt());
			...
			logger.logInfo("UserOperation", "userDataUpdate",
					"User data updated successfully for: " + ud.getUserName());
			return true;
