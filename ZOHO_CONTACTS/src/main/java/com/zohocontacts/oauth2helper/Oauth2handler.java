package com.zohocontacts.oauth2helper;
...
import com.zohocontacts.dataquerybuilder.querybuilderconfig.TableSchema.OauthProvider;
	...
	public static List<ContactDetails> getContacts(Oauth oauth) {
		...
		if (connections != null) {
			...
			for (int i = 0; i < connections.size(); i++) {
				...
				contact.setModifiedAt(Instant.now().toEpochMilli());
...
//						String formattedValue = addressObject.get("formattedValue").getAsString();
						if (addressObject.has("streetAddress")) {
						...
						if (addressObject.has("region")) {
							region = addressObject.get("region").getAsString();
						}
						if (addressObject.has("country")) {
							country = addressObject.get("country").getAsString();

						}
						if (addressObject.has("countryCode")) {
							countryCode = addressObject.get("countryCode").getAsString();
...
//						String postalCode = addressObject.get("postalCode").getAsString();
				...
				if (contactJson.has("metadata")) {
					JsonObject metadata = contactJson.getAsJsonObject("metadata");
					...
					if (metadata != null) {
						...
						if (sources != null) {
							...
							String oauthUpdatedTime = sources.get(0).getAsJsonObject().get("updateTime").getAsString();

							Instant instant = Instant.parse(oauthUpdatedTime);
							long oauthContactUpdatedTime = instant.toEpochMilli();
							contact.setOauthcContactModifiedTime(oauthContactUpdatedTime);
						}
